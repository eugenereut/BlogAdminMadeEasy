...
class Bin_Shelve extends Bin
{
	...
	private function wrap_to_htmlstr($arr_posts) {
		...
		if (!empty($arr_posts['ShelveName'])) {
			$_htmlstr .= '<h2>' . $arr_posts['ShelveName'] . ' · Все тексты</h2>';
		}
    ...
    if (!empty($arr_posts['modal_window']['pages'])) {
      ...
      if ($_next_less < 1) {
        ...
        $_htmlstr .= '<a name="'.$arr_posts['modal_window']['iDsh'].'" id="'.$_next_less.'" onclick="getpostin_modalwindow(this.name, this.id)" class="next">« Туда</a>';
      }

      $_htmlstr .= $_btngroup;

      if ($_next_more <= $arr_posts['modal_window']['pages']) {
        $_htmlstr .= '<a name="'.$arr_posts['modal_window']['iDsh'].'" id="'.$_next_more.'" onclick="getpostin_modalwindow(this.name, this.id)" class="next">Сюда »</a>';
      }
      else {
